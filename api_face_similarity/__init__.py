...
def create_app():
    ...
    if os.path.exists(root_path / 'config'):
        ...
        if os.environ.get('MODE') == 'prod':
            ...
            if not os.path.exists('/var/log/'):
                print('WARNING: Logging will not work since /var/log/ '
                      'do not exist.')
            else:
    ...
    # Error handler
    from api_face_similarity.utils.response_error import error_handler
    app.register_blueprint(error_handler)
    # Helpers controller
    from api_face_similarity.utils.helpers_controller import helpers
    app.register_blueprint(helpers)
    # Start controller
    from api_face_similarity.controller.start_controller import\
        start_route
    app.register_blueprint(start_route)
    # iiB CURIO controller
    from api_face_similarity.controller.iib_controller \
        import iib
    app.register_blueprint(iib)
    # Face distance controller
    from api_face_similarity.controller.face_distance_controller \
        import face_distance
    app.register_blueprint(face_distance)
    # Logger
    logging.getLogger('face_similarity.init').info(
